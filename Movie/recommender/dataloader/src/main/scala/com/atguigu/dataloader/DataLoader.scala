 ...
 * @param transportHosts Transport主机列表以‘，’分割
 * @param index          需要操作的索引
...
object DataLoader {
  ...
  val MOVIE_DATA_PATH = "C:\\Users\\Ming.Li\\Desktop\\Dongfanglong\\Movie\\recommender\\dataloader\\src\\main\\resources\\movies.csv"
  val RATING_DATA_PATH = "C:\\Users\\Ming.Li\\Desktop\\Dongfanglong\\Movie\\recommender\\dataloader\\src\\main\\resources\\ratings.csv"
  val TAG_DATA_PATH = "C:\\Users\\Ming.Li\\Desktop\\Dongfanglong\\Movie\\recommender\\dataloader\\src\\main\\resources\\tags.csv"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val config = Map(
      ...
      "es.httpHosts" -> "MovieR:9200",
      "es.transportHosts" -> "MovieR:9300",
      "es.index" -> "recommender",
      "es.cluster.name" -> "es-cluster"
    )
    //需要创建一个SparkConf配置
    val sparkConf = new SparkConf().setAppName("DataLoader").setMaster(config.get("spark.core").get)
    ...
    //首先需要将Tag数据集进行处理， 处理后的形式为MID， tag1|tag2|tag2
